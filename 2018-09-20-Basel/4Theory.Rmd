...
- An expression like $\|\mathbf y-\mathbf X\beta-\mathbf Z\Lambda\mathbf u\|^2 + \|\mathbf u\|^2$ is called a *penalized sum of squared
      residuals because $\|\mathbf y-\mathbf X\beta-\mathbf Z\Lambda\mathbf u\|^2$ is a sum of squared residuals and
...
- As described in Davis (2006), \S4.6, the calculation is
...
$d\mathbf v=\abs(|\mathbf L||\mathbf P|)\,d\mathbf u$, in
...
$$\begin{multline*}
      ...
      = \int\frac{\exp\left(\frac{-\|\mathbf v\|^2}{2\sigma^2}\right)}{(2\pi\sigma^2)^{q/2}}\,\frac{d\mathbf
        v}{\abs(|\mathbf L||\mathbf P|)} = \frac{1}{\abs(|\mathbf L||\mathbf
        P|)}=\frac{1}{|\mathbf L|}
    \end{multline*}$$

    because $\abs|\mathbf P|=1$ and $\abs|\mathbf L|$, which is the product of
...
$$L_R(\theta,\sigma^2|\mathbf y)=\int L(\theta,\beta,\sigma^2|\mathbf y)\,d\beta$$
    ...
    the integral w.r.t. $\mathbf u$ as $1/\abs(|\mathbf L|)$ produces 
    $1/\abs(|\mathbf R_X|)$ here and removes
