...
- Random variables
    ...
    - $\mathcal U$ the orthogonal random effects, such that $\mathcal B=\mathbf L^\prime\mathcal U$

- Parameters
    - $\beta$ - fixed-effects coefficients
    - $\sigma$ - the common scale parameter (not always used)
    - $\theta$ - parameters that determine $\mathrm{Var}(\mathcal B)=\sigma^2\mathbf L\mathbf L^\prime$ 

- Some matrices
    - $\mathbf{X}$ the $n\times p$ model matrix for $\beta$
    - $\mathbf{Z}$ the $n\times q$ model matrix for $\mathbf b$
    - $\mathbf L$ relative covariance factor as above
...
$$(\mathcal Y|\mathcal U=\mathbf u)\sim\mathcal{N}\left(\mu_{\mathcal Y|\mathcal U},\sigma^2\mathbf I\right)\text{ where }
      ...
      \mathbf X\beta+\mathbf Z\mathbf L\mathbf u$$
...
- When the univariate conditional distributions have constraints
    ...
    linear predictor, $\mathbf X\beta+\mathbf X\mathbf L\mathbf u$,
    which is unbounded.
...
- The linear predictor is $\mathbf g(\mu_{\mathcal Y|\mathcal U})=\eta=\mathbf X\beta+\mathbf Z\mathbf L\mathbf u$.
    Alternatively, we can write the conditional mean of
    $\mathcal Y$, given $\mathcal U$, as
$$\mu_{\mathcal Y|\mathcal U}(\mathbf u)= \mathbf g^{-1}\left(\mathbf X\beta+\mathbf Z\mathbf L\mathbf u\right)$$
...
$$\mathbf P\left(\mathbf L^\prime\mathbf Z^\prime\mathbf M\mathbf W\mathbf M\mathbf Z\mathbf L+\mathbf I_q\right)\mathbf P^\prime
      \delta_{\mathbf u}=\mathbf L^\prime\mathbf Z^\prime\mathbf M\mathbf W(\mathbf y-\mu) - \mathbf u$$
